...
int main(){
    ...
    vector <int*> step[2];
    int position[2]={0};
    position[0]=0;/*set position right+ and left-*/
    position[1]=0;/*set position up+ and down-*/
    ...
    pokemon.setattack(1000);
    pokemon.setHP(3000);
    pokemon.setHP_limit(3000);
    pokemon.setdefence(1000);
    pokemon.setevolving_status(0);
    pokemon.setwater(100);
    pokemon.setfood(100);
    pokemon.setpoison(0);
    pokemon.setbeat_monster(5);
    pokemon.setmoney(30);
    int p[2]={0,0};
    ...
    while(this_game){
        ...
        if(a==1){
            ...
            if(pokemon.showposition1()>0&&pokemon.showposition2()<0){
                int swamp_poison;
                srand(time(NULL));
                swamp_poison=(rand()%10+1);
                if(swamp_poison==1){
                    cout<<"You are poisoned by swamp"<<endl;
                    pokemon.setpoison(pokemon.showpoison()+10);
                }
